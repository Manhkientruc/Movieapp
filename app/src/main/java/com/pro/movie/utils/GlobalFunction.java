...
public class GlobalFunction {
    ...
    public static UserInfor getUserInfor(Movie movie) {
        ...
        for (UserInfor userInfoObject : listFavorite) {
            if (DataStoreManager.getUser().getEmail().equals(userInfoObject.getEmailUser())) {
                userInfor = userInfoObject;
                break;
            }
        }
        return userInfor;
    ...
    public static void onClickFavoriteMovie(Context context, Movie movie, boolean isFavorite) {
        ...
        UserInfor userInfoFavorite = getUserInfor(movie);
        if (userInfoFavorite != null) {
            MyApplication.get(context).getMovieDatabaseReference()
                    .child(String.valueOf(movie.getId()))
                    .child("favorite")
                    .child(String.valueOf(userInfoFavorite.getId()))
                    .removeValue();
        }
    }
}
